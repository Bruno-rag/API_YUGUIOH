...
// Busca cartas por nível
async function buscarPorNivel(nivel) {
    try {
        const apiUrl = `https://db.ygoprodeck.com/api/v7/cardinfo.php?level=${nivel}`;
...
// Busca e exibe cartas por nível
function buscarCartas() {
    ...
    if (!nivelSelecionado) {
        alert("Por favor, selecione um nível de carta.");
        return;
    }

    buscarPorNivel(nivelSelecionado)
        .then(data => {
            if (data.data && data.data.length > 0) {
                ...
                noResultMessage.textContent = 'Nenhum card encontrado com esse nível.';
