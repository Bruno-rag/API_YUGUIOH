...
async function buscarPorTipo(tipo) {
    try {
        const apiUrl = `https://db.ygoprodeck.com/api/v7/cardinfo.php?type=${tipo}`;
...
// Função principal para buscar e exibir cartas por tipo
function buscarCartas() {
    ...
    if (!tipoSelecionado) {
        alert("Por favor, selecione um tipo de carta.");
        return;
    }

    buscarPorTipo(tipoSelecionado)
        .then(data => {
            if (data.data && data.data.length > 0) {
                ...
                noResultMessage.textContent = 'Nenhum card encontrado com esse tipo.';
